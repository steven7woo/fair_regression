...
def plot_multiples_test():
    fig, ax = plt.subplots(nrows=2, sharex=True, sharey=True, figsize=(8, 18))
    ...
    plt.plot([0.1], [100], color='orange',linewidth=2,  markersize=5, label='fair reg. (oracle=CS, class=linear)')
    plt.plot([0.1], [100], color='red', linewidth=2, markersize=5, label='fair reg. (oracle=LS, class=linear)')
    ...
    plt.errorbar([0.1], [100], xerr=[0.0], marker='^', markeredgecolor
                 ...
                 label='unconstrained reg. (class=linear)', ecolor='black',
                 capthick=1, markersize=11, capsize=2)
    plt.errorbar([0.1], [100], xerr=[0.0], marker='^', markeredgecolor
                 = 'black', color='black', markerfacecolor='green',
                 label='unconstrained reg. (class=tree ensemble)', ecolor='black',
                 capthick=1, markersize=11, capsize=2)

    plt.scatter([0.1], [100], marker='v', color='deepskyblue', edgecolors = 'black', s=95,
                label='SEO (class=linear)')

    plt.legend(loc=10, bbox_to_anchor=(-2.1, 0.5), fontsize=10)
    plt.savefig('test.pdf', bbox_inches='tight')
    plt.show()
